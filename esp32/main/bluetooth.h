...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param) {
    ...
    case ESP_GAP_BLE_ADV_START_COMPLETE_EVT:
        ESP_LOGD(BLUETOOTH_TAG, "Advertising started, status=%d", param->adv_start_cmpl.status);
        break;
    default:
        break;
    }
...
static void gatts_event_handler(esp_gatts_cb_event_t event,
    ...
    ESP_LOGD(BLUETOOTH_TAG, "gatts_event_handler called: event=%d, gatts_if=%d", event, gatts_if);
    switch (event) {
    ...
    case ESP_GATTS_CREAT_ATTR_TAB_EVT:
        ESP_LOGD(BLUETOOTH_TAG, "ESP_GATTS_CREAT_ATTR_TAB_EVT: status=%d", param->add_attr_tab.status);
    ...
    case ESP_GATTS_WRITE_EVT:
        ESP_LOGD(BLUETOOTH_TAG, "ESP_GATTS_WRITE_EVT: handle=0x%04x, len=%d", param->write.handle, param->write.len);
...
static void bluetooth_app_start(void) {
    ...
    if (ret) {
        ESP_LOGE(BLUETOOTH_TAG, "controller init failed: %s", esp_err_to_name(ret));
        return;
    }
    ESP_LOGD(BLUETOOTH_TAG, "BT controller initialized");
    ret = esp_bt_controller_enable(ESP_BT_MODE_BLE);
    if (ret) {
        ESP_LOGE(BLUETOOTH_TAG, "controller enable failed: %s", esp_err_to_name(ret));
        return;
    }
    ESP_LOGD(BLUETOOTH_TAG, "BT controller enabled");
    ret = esp_bluedroid_init();
    if (ret) {
        ESP_LOGE(BLUETOOTH_TAG, "bluedroid init failed: %s", esp_err_to_name(ret));
        return;
    }
    ESP_LOGD(BLUETOOTH_TAG, "Bluedroid initialized");
    ret = esp_bluedroid_enable();
    if (ret) {
        ESP_LOGE(BLUETOOTH_TAG, "bluedroid enable failed: %s", esp_err_to_name(ret));
        return;
    }
