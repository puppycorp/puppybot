...
static void websocket_send_my_info(void) {
	...
	esp_err_t err = esp_websocket_client_send_bin(client, (const char *)payload,
	                                              offset, portMAX_DELAY);
	if (err != ESP_OK) {
		ESP_LOGE(TAG, "Failed to send MyInfo message: %s",
		         esp_err_to_name(err));
	} else {
		ESP_LOGI(TAG, "Sent MyInfo (fw=%.*s, variant=%.*s)", (int)version_len,
...
static void heartbeat_timer_callback(void *arg) {
	...
	if (websocket_connected && client) {
		...
		esp_err_t err =
		    esp_websocket_client_send_text(client, "ping", 4, portMAX_DELAY);
...
static esp_err_t ws_httpd_handler(httpd_req_t *req) {
	...
	if (ret != ESP_OK) {
		ESP_LOGE(TAG, "Failed to get WS frame length: %s",
		         esp_err_to_name(ret));
		return ret;
	}
	...
	case HTTPD_WS_TYPE_BINARY:
		...
		if (cmd_packet.cmd_type == CMD_PING) {
			...
			if (ret != ESP_OK) {
				ESP_LOGE(TAG, "Failed to send protocol pong: %s",
				         esp_err_to_name(ret));
			}
		}
		break;
	case HTTPD_WS_TYPE_TEXT:
		ESP_LOGI(TAG, "WS text frame: %s",
		         ws_pkt.payload ? (char *)ws_pkt.payload : "<empty>");
		break;
	...
	case HTTPD_WS_TYPE_CLOSE: {
		...
		if (ret != ESP_OK) {
			ESP_LOGE(TAG, "Failed to acknowledge close frame: %s",
			         esp_err_to_name(ret));
		}
		break;
	}
...
static void websocket_server_start(void) {
	...
	if (ret == ESP_OK) {
		ESP_LOGI(TAG, "WebSocket server started on port %d",
		         config.server_port);
		httpd_register_uri_handler(ws_server, &ws_uri_handler);
	} else {
		ESP_LOGE(TAG, "Failed to start WebSocket server: %s",
		         esp_err_to_name(ret));
		ws_server = NULL;
	}
}
...
void websocket_event_handler(void *handler_args, esp_event_base_t base,
	...
	case WEBSOCKET_EVENT_DATA:
		...
		if (data->data_len == 4 &&
		    strncmp((char *)data->data_ptr, "pong", 4) == 0) {
			ESP_LOGI(TAG, "Received heartbeat pong");
			break;
		}
		if (data->data_len == 4 &&
		    strncmp((char *)data->data_ptr, "ping", 4) == 0) {
...
bool is_websocket_connected(void) { return websocket_connected; }

void websocket_send_status(void) {
	if (websocket_connected && client) {
		const char *status_msg = "status_ok";
		esp_err_t err = esp_websocket_client_send_text(
		    client, status_msg, strlen(status_msg), portMAX_DELAY);
		if (err != ESP_OK) {
...
void websocket_app_start() {
	...
	const esp_timer_create_args_t heartbeat_timer_args = {
	    .callback = heartbeat_timer_callback, .name = "websocket_heartbeat"};
	esp_err_t ret = esp_timer_create(&heartbeat_timer_args, &heartbeat_timer);
	if (ret != ESP_OK) {
		ESP_LOGE(TAG, "Failed to create heartbeat timer: %s",
		         esp_err_to_name(ret));
	}
