...
static void mock_ensure(uint8_t channel, uint16_t freq_hz) {
        (void)channel;
        if (!g_active_mock)
                return;
...
static void mock_bind(uint8_t channel, int gpio) {
        (void)channel;
        (void)gpio;
        if (!g_active_mock)
                return;
...
static void mock_set_pulse(uint8_t channel, uint16_t freq_hz, uint16_t pulse_us) {
        (void)channel;
        if (!g_active_mock)
                return;
...
static void mock_set_duty(uint8_t channel, float duty_ratio) {
        (void)channel;
        if (!g_active_mock)
                return;
...
static void mock_configure_hbridge(int in1, int in2, bool forward, bool brake) {
        (void)in1;
        (void)in2;
        if (!g_active_mock)
                return;
...
static void mock_hw_setup(mock_hw_t *mock) {
        ...
        mock->ops.init = mock_init;
        mock->ops.ensure_pwm = mock_ensure;
        mock->ops.bind_pwm_pin = mock_bind;
        mock->ops.set_pwm_pulse_us = mock_set_pulse;
        mock->ops.set_pwm_duty = mock_set_duty;
        mock->ops.configure_hbridge = mock_configure_hbridge;
        mock->ops.now_ms = mock_now;
        g_active_mock = mock;
        motor_runtime_set_hw_ops(&mock->ops);
}

static void mock_hw_teardown(void) {
        g_active_mock = NULL;
        motor_runtime_set_hw_ops(NULL);
}
...
TEST(motor_registry_add_records_time_from_hw_ops) {
...
TEST(motor_set_speed_invokes_hw_ops_for_hbridge) {
        motor_registry_clear();
        mock_hw_t mock;
        mock_hw_setup(&mock);
        mock.now_value = 10;
