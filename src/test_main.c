...
int main(int argc, char **argv) {
	...
	for (Test *t = test_list; t; t = t->next) {
		if (fnmatch(filter, t->name, 0) == 0) {
			...
			printf(COLOR_CYAN "[%d] Running %-*s ... " COLOR_RESET, test_index,
			       max_name_len, t->name);
			clock_t start = clock();
			t->func();
			clock_t end = clock();
	...
	if (failure_list) {
		...
		for (TestFailure *f = failure_list; f; f = f->next)
			printf(COLOR_RED "%s: %s, in %s:%d\n" COLOR_RESET, f->test_name,
			       f->expr, f->file, f->line);
	}
	clock_t total_end = clock();
	double total_elapsed =
	    (double)(total_end - total_start) / CLOCKS_PER_SEC * 1000;
	int total_passed = total_tests_run - total_tests_failed;
	printf("\nSummary: Total: %d, " COLOR_GREEN "Passed: %d" COLOR_RESET
	       ", " COLOR_RED "Failed: %d" COLOR_RESET "\n",
